...
test("sqs publisher", async (t) => {
	...
	await t.test("SQS publish message", async (t) => {
		...
		// biome-ignore lint/style/noNonNullAssertion: ok if fails
	...
	await t.test("SQS publish message w attributes", async (t) => {
		...
		await app.ready();
		const publisherMessage = new PublisherMessage(
			{ message: "test-sqs" },
			{
	...
	await t.test("SQS publish message w attributes wrong data", async (t) => {
		...
		try {
			...
			t.same((e as Error).message, "Invalid data type");
		}
	});
	await t.test("SQS publish delayed message", async (t) => {
		...
		await app.ready();
		await app.delayedMessageToSQS(
			{ message: "test-sqs", date: Date.now() },
			queueARN,
			1,
		);
		const messages = await sqs.receiveMessage(queueARN, {
		...
		const message = messages.Messages?.[0];
		// biome-ignore lint/style/noNonNullAssertion: is a test
	...
	await t.test("SQS publish delayed message as text", async (t) => {
		...
		await app.ready();
		await app.delayedMessageToSQS(
			JSON.stringify({ message: "test-sqs", date: Date.now() }),
			queueARN,
			1,
		);
		const messages = await sqs.receiveMessage(queueARN, {
	...
	await t.test("SQS publish messages", async (t) => {
		...
		t.teardown(async () => {
			await app.close();
			await sqsPurge(
				`${process.env.LOCALSTACK_ENDPOINT}/000000000000/test-queue`,
			);
		});
		app.register(publisherPlugin, {
			sqs: {
		...
		const queueAttributes = await new SQS({
			endpoint: process.env.LOCALSTACK_ENDPOINT,
		}).getQueueAttributes({
	...
	await t.test("SQS publish messages w attributes", async (t) => {
		...
		await app.ready();
		const messages: PublisherMessage[] = [];
		for (let i = 0; i < 11; i++) {
			const publisherMessage = new PublisherMessage(
				{ message: "test-sqs-batch" },
				{
					"test-attribute-number": 1,
					"test-attribute-string": "test",
				},
	...
	await t.test("SQS publish messages delayed", async (t) => {
		...
		for (let i = 0; i < 11; i++) {
			messages.push(
				JSON.stringify({ message: "test-sqs-batch", date: Date.now() }),
			);
		}
	...
	await t.test("SQS publish delayed messages w attributes", async (t) => {
		...
		for (let i = 0; i < 11; i++) {
			const publisherMessage = new PublisherMessage(
				{ message: "test-sqs-batch", date: Date.now() },
				{
					"test-attribute-number": 1,
					"test-attribute-string": "test",
				},
